"""
Benchmark : Qwen3.5-27B (Quantizé 4-bit via HuggingFace)
=========================================================
...
logger = logging.getLogger("benchmark_qwen27b")

MODELE_ID = "./langage/modeles/Qwen3.5-27B"
OUTPUT_FILE = os.path.join(os.path.dirname(__file__), "resultats_qwen27b_quantize.json")
...
class ResultatBenchmark:
    ...
    quantification: str = "4-bit (BitsAndBytes)"
    dtype: str = "bfloat16"
...
def main() -> None:
    ...
    logger.info("Chargement de l'Agent et du modèle %s (4-bit)...", MODELE_ID)
    t0 = time.time()
    
    # Instance du ModelService customisé pour 4-bit
    service = ModelService(model_id=MODELE_ID, load_in_4bit=True)
