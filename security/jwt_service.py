...
def create_access_token(user_id : int , card_id : str ) -> str : 
    payload = {
        ...
        "card_id": card_id,
        "iat":datetime.utcnow(),
        "exp": datetime.utcnow() + timedelta(minutes=CONFIGURATION.jwt.access_token_expire_minutes)
    }
    token = jwt.encode(payload, CONFIGURATION.jwt.secret_key, algorithm=CONFIGURATION.jwt.algorithm)
    return token

def verify_access_token(token : str) -> dict :
    try : 
        payload = jwt.decode(token, CONFIGURATION.jwt.secret_key, algorithms=[CONFIGURATION.jwt.algorithm])
...
def create_refresh_token(user_id : int , card_id : str ) -> str : 
    ...
    expires_at = datetime.utcnow() + timedelta(minutes=CONFIGURATION.jwt.refresh_token_expire_minutes)
